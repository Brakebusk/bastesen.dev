...
function loadDirectories(filename) {
    ...
    xmlhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            //Add welcome message:
            addOutput(navigate("/etc", false)["files"]["motd"]["content"], "10px");
        }
    };
...
function onloadPrep() {
    ...
    loadDirectories("directory.min.json"); //Load directory structure
...
function addToLog(command) {
    ...
    elem.textContent = "guest@h4ck32m4n:" + _path + "$ " + command;
...
function updatePathLabels() {
    ...
    let _path = path;
    if (path.slice(0, 11) == "/home/guest") _path = "~" + path.slice(11);
    document.getElementById("commandPath").textContent = "guest@h4ck32m4n:" + _path + "$ ";
}
...
function runScript(command) {
    //Lazy parsing
    if (path=="/home/hackerman" && command == "./decryptor" || command == "./decryptor.sh") {
        script = atob(navigate('', false)["files"]["decryptor.sh"]["content"]);
        eval(script);
    }
}
...
function handle_ls(command) {
    ...
    if (!verbose) {
        ...
        for (var i = 0; i < directories.length; i++) {
            ...
            output += "drwxrwxrwxr 1 guest guest " + " ".repeat(padding) + "512 " + edited + " " +  directories[i];
        }
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            output += "-rwxrwxrwxr 1 guest guest " + " ".repeat(padding) + size + " " + edited + " " +  files[i];
        }
